...
it is important to distinguish the language that is being talked about,
...
Thus a sentence letter has complexity 0;
...
\begin{exercise}
  ...
  the initial symbol of any $\L_{0}$-sentence $A$
  never belongs to an $\L_{0}$-sentence that is a proper part of $A$.
  (A \emph{proper part} of a string is a substring that is not the whole string itself.)
  % [Bostock pp52f.]
\end{exercise}
...
Next,
let's explain how one may reason in $\L_{0}$.
...
\begin{definition}{}{proof0}
  ...
  A proof is \emph{of} an $\L_{0}$-sentence $A$
  if $A$ is the last sentence in the sequence.
\end{definition}

We write `$\vdash_{0} A$' to express that
...
For example,
`$\{ p,q \} \vdash_{0} p$' is a sentence in our metalanguage saying that $p$ is deducible from the set containing $p$ and $q$.
We'll usually omit the set braces and simply write `$p,q \vdash_0 q$'.
...
\begin{exercise}
  Explain why Id, Mon, and Cut follow from definition~\ref{def:deduction0},
...
Being finite,
this deduction must use only finitely many premises from $\Gamma$.
...
\]
($A_{1} \to (A_{2} \to (\ldots (A_{n} \to B)\ldots))$ is provably equivalent to
$(A_{1} \land A_{2} \land \ldots \land A_{n}) \to B$,
so we also get $\vdash_{0} (A_{1} \land A_{2} \land \ldots \land A_{n}) \to B$.)
...
\begin{proof}
  ...
  \]
  Using A2,
  the deduction of $A \to B_{i}$ and $A \to (B_{i} \to B_{k})$ from $\Gamma$
  can therefore be extended by MP to a deduction of $(A \to B_{i}) \to (A \to B_{k})$
  and from there to $A \to B_{k}$.
  \qed
\end{proof}

The proof uses axioms A1 and A2.
...
\quad 2.\quad & A, B \vdash_0 A &&\text{(Mon)}& \\
...
I've introduced `$\Gamma \vdash_{0} A$' to mean
...
These are, in effect,
upside-down sequent proofs in which
...
a proof is simply a list of $\L_{0}$-sentences,
...
In this course,
we'll focus on classical logic.
...
\begin{exercise}
  Give a sequent calculus proof of \emph{Peirce's Law}: $p \to ((p \to q) \to p)$.
  The proof requires DNE, although the sentence doesn't contain any negation.
\end{exercise}
...
The kind of theory we're going to study instead belongs to the tradition of \textit{truth-conditional semantics}.
...
Next,
we explain how
an assignment of truth-values to sentence letters determines
...
\begin{exercise}
  Explain why, if $\Gamma \models A$ by definition~\ref{def:entailment0},
  then $\Gamma$ entails $A$ according to the earlier, informal definition.
\end{exercise}
...
\begin{exercise}
  We have now introduced five arrow-like symbols:
...
\section{Soundness and Completeness}\label{prop-completeness}
...
We studied proofs and deductions as arrangements of symbols conforming to certain rules,
...
\begin{proof}
  ...
  $\Gamma \entails A_k$.
  The theorem then follows by taking $k = n$.
