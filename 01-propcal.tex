...
\begin{definition}{Deduction}{deduction}
  A (propositional) \emph{deduction} of a sentence A from a set Γ ("gamma") of
  sentences is a finite sequence of sentences whose last element is A and each
  of which is either an axiom, an element of Γ, or follows from previous
  sentences by MP.
\end{definition}

We write Γ $\vdash_{0}$ A to mean that there is a deduction of A from Γ. So $\{ p,q \} \vdash_{0}
...
empty set of premises. We write $\vdash_{0} A$ to say that there is a proof of $A$ in
our calculus.

% ** Structural properties of deductions

The following "structural principles" immediately follow from definition xxx.
...
$\Delta \cup \Gamma$ is the \textit{union} of $\Gamma$ and $\Delta$. (The union of two sets is the set that contains all and only elements that are in either of the sets.) To remove cutter, we will henceforth remove the parentheses and write a comma instead of $\cup$. We'll also omit the metalinguistic quantifiers over sentences. I.e., we'll write

Assumptions: $A \vdash A$
...
...
The fact that MP can be used to extend deductions means that we also have the
following principle:
...
Note that we needed MP and ax.1 and ax.2. From the DT and MP, one can prove ax 1
and ax 2.
...
\begin{exercise}
Show in the same way that A1 and A2 are provable. [Bostock 206f.]
\end{exercise}

The exercise shows that if we're only interested in whether a deduction or proof exists, we never need to look at A1 and A2 any more.
