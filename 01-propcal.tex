...
% xxx clarify "subset" doesn't mean proper subset!
...
It doesn't matter
(as long as none of them has any other primitive $\L_{0}$-symbol as a part,
...
\end{itemize*}
%
I could have added `$\land$', `$\lor$', and `$\leftrightarrow$' as primitive symbols;
...
\begin{cenumerate}
  ...
  \item[(ii)] \emph{If} every sentence of complexity below $n$ has the property \emph{then} so does every sentence of complexity $n$.
\end{cenumerate}
%
Step (i) is called the \emph{base case} of the proof;
step (ii) the \emph{inductive step}.
The antecedent of (ii),
that every sentence of complexity below $n$ has the property,
...
we use induction on the lines in a proof.
We assume that
there is a deduction $B_{1}, B_{2}, \ldots, B_{n}$ of $B$ from $\Gamma \cup \{ A \}$.
We then show that $\Gamma \vdash_{0} A \to B_{k}$ for each $k = 1, 2, \ldots, n$.
Since $B_{n} = B$,
this will show that $\Gamma \vdash_{0} A \to B$.
To show that $\Gamma \vdash_{0} A \to B_{k}$ for each $k = 1, 2, \ldots, n$,
it suffices to show that
\begin{cenumerate}
  \item[(i)] $\Gamma \vdash_{0} A \to B_{1}$, and
  \item[(ii)] \emph{if} $\Gamma \vdash_{0} A \to B_{i}$ for all $0 < i < k$, \emph{then} $\Gamma \vdash_{0} A \to B_{k}$.
\end{cenumerate}
In fact,
(ii) alone is sufficient.
To see this,
consider the case $k = 1$.
Since there is no $i$ with $0 < i < 1$,
the \emph{if}-part of (ii) is vacuously true.
If we can show (ii),
we can therefore also show that $\Gamma \vdash_{0} A \to B_{1}$.
...
\begin{proof}
  ...
  We will show that
  \emph{if} $\Gamma \vdash_{0} A \to B_{i}$ for all $0 < i < k$,
...
\begingroup
\allowdisplaybreaks % allow page breaks in flalign*
...
% mirroring what you've established in exercise xyz for the case of `$\land$'.)
