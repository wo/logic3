...
which shows that
whenever a first-order sentence is entailed by some premises
then it is derivable from these premises in the first-order calculus.
...
These theorems concern the \emph{size} of models,
...
$\{$ Athens $\}$ is a set with one member:
the city Athens.
...
But now consider the set $\mathbb{N} = \{ 0, 1, 2, 3, \ldots \}$ of natural numbers.
What is its size?

The official set-theoretic word for the size of a set is \textit{cardinality}.
So $\{$ Athens, Berlin, Cairo $\}$ has cardinality 3.
Finite sets have finite cardinalities, which are natural numbers.
...
\begin{definition}{}{bijection}
  ...
  \begin{cenumerate}
    ...
    \item[(ii)] For every $y \in B$ there is some $x \in A$ such that $f(x) = y$. (Surjectivity)
  \end{cenumerate}
\end{definition}
...
One might suspect that the set of \emph{pairs} of natural numbers is uncountable.
But not so.
...
\begin{exercise}
  ...
  % Can zig zag the cube. But easier: zig zag a 2D grid in which the rows are the pairs $(x,y)$ of natural numbers and the columns are natural numbers. We know that the pairs can be enumerated.
\end{exercise}
...
\begin{proof}
  ...
  This means there is a list $S_0, S_1, S_2, \ldots$ of all these sets.
  Now consider the set $D = \{ n \in \mathbb{N} : n \notin S_n \}$
  of all natural numbers $n$ that are not in the $n$-th set $S_n$ of the list.
...
Cantor's method now looks at the shaded diagonal.
...
It can't be anywhere in the list $S_0, S_1, S_2, \ldots$.
So there can be no list of all sets of natural numbers.

Theorem \ref{thm:cantor} can be strengthened:

\begin{theorem}{Cantor's Theorem (general version)}{cantor2}
  For any set $A$,
...
\begin{proof}
  ...
  Suppose for reductio that there is a bijection $f$ from  $A$ to $\mathcal{P}(A)$,
...
\begin{exercise}
  Show that if a first-order language has countably many primitive symbols,
  then the set of all sentences of the language is countably infinite.
\end{exercise}

\begin{exercise}
  Show that the set of real numbers is uncountable.
  (Hint: use the diagonalization method,
  assuming, for reductio, that there is a list of all
  decimal representations of real numbers between 0 and 1.)
\end{exercise}
...
where every subset of $D$ is a possible interpretation of any monadic predicate.
...
The number of first-order models is vastly greater than $\aleph_0$.
...
Let's grant, for the sake of the argument, that
...
so it will answer this question.
...
(which is short for $\neg \forall x \neg Fx$).
We want all sentences in $\Gamma$ to be true in our model $\Mod{M}$.
...
\begin{exercise}\label{ex:henkin-quantifiers-prf}
  Show that if $\Gamma$ is a Henkin set in some language $\L$ then
  $\Gamma$ contains $\forall x A$ iff
  $\Gamma$ contains $A(x/c)$ for every individual constant $c$ in $\L$.
\end{exercise}
...
\begin{exercise}
  Which of these are equivalence relations on the set of natural numbers?
  (a) $x R_1 y$ iff $x > y$;
  \, (b) $x R_2 y$ iff $x > y$ or $y > x$;
  \, (c) $x R_3 y$ iff there are natural numbers $m,n$ such that $x \times 2^m = y \times 2^n$.
...
\begin{proof}
  \emph{Proof.}
  Assume $[s_{i}] = [t_{i}]$ for each $i=1,2,\ldots,n$.
  That is,
  $\Gamma$ contains $s_{i}\!=\!t_{i}$, for each $i=1,2,\ldots,n$.

  (i).
  By A6 (and exercise~\ref{ex:henkin-closure}),
  $f(s_{1},\ldots,s_{n})=f(s_{1},\ldots,s_{n})$ is in $\Gamma$.
  By $n$ instances of A7 and MP (and exercise~\ref{ex:henkin-closure}),
...
\begin{definition}{}{henkin-model}
  ...
  \smallskip
  The domain $D$ of $\Mod{M}_{\Gamma}$ is the set $\{ [c] \,:\, c \text{ is an individual constant of } \L \}$.
  ...
  \begin{cenumerate}
    ...
    \item[(ii)] each $n$-ary function symbol $f$ to the function on $D$ that maps
         any $n$ objects $[t_1],\ldots,[t_n]$ in $D$ to $[f(t_1,\ldots,t_n)]$,
...
\begin{proof}
  ...
  \begin{align*}
    \llbracket f(t_{1},\ldots,t_{n})\rrbracket^{\Mod{M}} &= \llbracket f\rrbracket^{\Mod{M}}(\llbracket t_{1}\rrbracket^{\Mod{M}},\ldots,\llbracket t_{n}\rrbracket^{\Mod{M}}) &&\text{ by def.\ \ref{def:satisfaction=}}\\
                               ...
                               &= [f(t_{1},\ldots,t_{n})] &&\text{ by def.~\ref{def:henkin-model}.(iii)} \qed
  \end{align*}
\end{proof}
...
also known as the \emph{standard model} of arithmetic, or $\Mod{N}$,
has as its domain
...
Let $\mathrm{Th}(\Mod{N})$ be the set of all sentences true in
the standard model $\Mod{N}$.
This is called the \emph{theory of $\Mod{N}$}.
It contains all postulates we could possibly lay down,
assuming that they should all be true in the standard model.
By compactness,
there is a model of $\mathrm{Th}(\Mod{N})$ whose domain
...
\begin{theorem}{Non-standard Models of Arithmetic}{non-standard-arithmetic}
  $\mathrm{Th}(\Mod{N})$ has models in which
...
\begin{proof}
  ...
  Let $\mathrm{Th}(\Mod{N})^+$ be the set of sentences that
  extends $\mathrm{Th}(\Mod{N})$ by
  \[
    c \ne 0, \quad c \ne s(0), \quad c \ne s(s(0)), \quad c \ne s(s(s(0))), \quad\ldots.
  \]
  So $\mathrm{Th}(\Mod{N})^+$ says that
  $c$ is different from 0, 1, 2, and so on, for all natural numbers.
  Every finite subset of $\mathrm{Th}(\Mod{N})^+$ is obviously satisfiable:
  it is true in the standard model $\Mod{N}$,
  interpreting $c$ as a sufficiently large number.
  By the compactness theorem,
  it follows that $\mathrm{Th}(\Mod{N})^+$ is satisfiable.
  Any model of $\mathrm{Th}(\Mod{N})^+$
...
Here is a simpler example of this kind of limitation.
For each $n>0$,
...
\begin{proof}
  ...
  So the domain of the Henkin model is countable.
  \qed
\end{proof}
...
It entails,
among other things,
that $\mathrm{Th}(\Mod{N})$ has non-standard models of every infinite cardinality.
...
\begin{exercise}
  Let $\Mod{N}^{c}$ be exactly like the standard model of arithmetic $\Mod{N}$,
  except that the number 2 is replaced by Julius Caesar:
  the domain of $\Mod{N}^c$ is $\{ 0, 1, \text{Caesar}, 3, 4, \ldots \}$;
  ...
  and so on.
  Is $\Mod{N}^{c}$ a model of $\mathrm{Th}(\Mod{N})$?
...
% This model satisfies all sentences in $\mathrm{Th}(\Mod{N})$,
...
% the kind of structure that $\Mod{N}$ and $\Mod{M}^{jc}$ have in common.
...
That is,
there is no way to spell out axioms and rules of inference that define a proof relation $\proves_{\Mod{N}}$ so that
$\Gamma \proves_{\Mod{N}} A$ iff $\Gamma$ arithmetically entails $A$
