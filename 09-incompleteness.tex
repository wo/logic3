...
In Section~\ref{sec:decidable},
I introduced these concepts informally,
using the pre-theoretical notions of computability and algorithms.
We can now give more formal definitions.

We say that
a set of numbers is \emph{recursively decidable}
if the property of being in the set is a recursive relation,
as defined in Chapter~\ref{ch:recursive};
a set of sentences is recursively decidable
if the set of their code numbers is recursively decidable.
A theory is \emph{recursively axiomatizable} if
there is a recursively decidable set of axioms
from which all and only the sentences in the theory can be deduced.
By Theorem~\ref{thm:total-recursive-turing},
this is equivalent to saying that
there is a Turing machine that
determines whether a given $\L_A$-sentence is among the axioms or not.

The syntactic version of Gödel's First Incompleteness Theorem can now be stated as follows:
\emph{every consistent and recursively axiomatizable theory of arithmetic that is at least as strong as Q is incomplete.}

\begin{exercise}
  Let $T_1$ be the set of all $\L_A$-sentences.
  Is $T_1$ (a) a theory? (b) recursively axiomatizable? (c) complete? (d) consistent? (Explain.)
\end{exercise}

\begin{exercise}
  Let $T_2$ be the set of $\L_A$-sentences that are valid in first-order logic.
  Is $T_2$ (a) a theory? (b) recursively axiomatizable? (c) complete? (d) consistent?
\end{exercise}

\begin{exercise}
  Can you find an $\L_A$-theory that is recursively axiomatizable, complete, and consistent?
...
We've shown in Theorem~\ref{thm:definability} that
every computable property or relation is definable in $\L_A$.
...
so that we can indirectly refer an $\L_A$-string by the $\L_A$-numeral of its code.
...
\toprule
Symbol: & $0$ & $s$ & $+$ & $\times$ & $=$ & $\neg$ & $\to$ & $\forall$ & $($ & $)$ & $,$ & $x_{1}$ & $c_{1}$ & $x_2$ & $c_2$ & $\ldots$ \\
\midrule
...
Then we use the prime exponent method to code sequences of symbol codes,
...
% So we know from Theorem~\ref{thm:definability} that
...
\emph{defines} the property of coding a variable.
By Theorem~\ref{thm:definability},
every recursive relation and function is definable in $L_A$.
We can use this result 
to show that a wide range of syntactic notions are definable in $L_A$.
...
that relation or function is recursive.
By Theorem~\ref{thm:definability},
it is definable in $L_A$.
...
By the Church-Turing Thesis,
it follows that
the property (call it $\mathrm{Sent}$) of coding an $\L_A$-sentence is recursive.
By Theorem~\ref{thm:definability},
it follows that
...
an axiom of $T$,
an instance of the logical axioms A1--A6, or
...
it is recursive.
By Theorem~\ref{thm:definability},
it follows that it is definable in $L_A$:
...
By the Church-Turing Thesis,
the corresponding function on Gödel numbers is recursive:
there is a recursive function $*$ that maps
...
I've appealed to the Church-Turing Thesis to argue that $\mathrm{Sent}$, $\mathrm{Prf}_T$
and $*$ are recursive,
but we could have shown this directly:
we could show that $\mathrm{Sent}$, $\mathrm{Prf}_T$ and $*$ can be constructed
...
% is recursive,
% and therefore definable in $L_A$.
% The same is true if
...
and Gödel numerals $\gn{A(x)}$ (instead of quote marks) to refer to these predicates.
For example,
if $\smallcaps{Var}(x)$ is the formula that defines the property of coding a variable,
...
and easily definable with the concatenation function $*$:
\[
...
By Theorem~\ref{thm:definability},
all recursive functions are definable in $\L_A$.
...
\begin{lemma}{The Semantic Diagonal Lemma}{diagonallemma}
...
\begin{proof}
  ...
  Since $\smallcaps{diag}$ defines $\mathrm{diag}$,
...
Let $T$ be some recursively axiomatizable theory in $\L_A$,
so that 
there is a recursively decidable set of axioms $\Gamma$ from which all and only the members of $T$ can be deduced.
...
\begin{theorem}{Gödel's First Incompleteness Theorem, semantic version}{goedel1}
  Every sound and recursively axiomatizable $\L_A$-theory is incomplete.
\end{theorem}
\begin{proof}
  \emph{Proof.}
  Let $T$ be a recursively axiomatizable $\L_A$-theory.
...
\begin{exercise}
  ...
  there is a true sentence $G$ that is not provable in a sound and recursively axiomatizable theory such as PA.
...
rather than on its definability.
...
Now assume that $T$ is a recursively axiomatizable theory in which
...
\begin{cenumerate}
    ...
    \item[(i)] for every number $n$, $\vdash_T \neg A(\num{n})$.
\end{cenumerate}
...
\begin{theorem}{Gödel's First Incompleteness Theorem}{goedel1b}
  Every recursively axiomatizable and $\omega$-consistent theory in which
  all recursive functions are representable is incomplete.
\end{theorem}
% \begin{proof}
%   \emph{Proof.}
...
Since representability in $\mathrm{Th}(\Mod{N})$ implies definability in $\L_A$,
it follows that no $\L_A$-formula defines membership in $\mathrm{Th}(\Mod{N})$:

\begin{theorem}{}{tarski2}
  Arithmetical truth is not definable in $\L_A$:
  there is no $\L_A$-formula $T(x)$ such that
  $\Mod{N} \satisfies T(\gn{A})$ iff $\Mod{N} \satisfies A$.
\end{theorem}
\begin{proof}
  \emph{Proof.}
  $\mathrm{Th}(\Mod{N})$ is consistent extension of Q.
  ...
  \end{cenumerate}
  So there is no $\L_A$-formula that defines truth in $\Mod{N}$. 
  \qed
\end{proof}
...
(we can define $\L_A$-properties like being a variable or being a sentence),
...
guaranteed by the fact that (for suitable choices of $T$) $\mathrm{Prf}_T$ is recursive.
...
We begin with two small lemmas.
...
\begin{proof}
  \emph{Proof.}
  Let $T$ be a consistent theory in which all recursive functions are representable.
  By Theorem~\ref{thm:tarski},
  membership in $T$ is not representable in $T$.
  So membership in $T$ is not recursive:
  the set of Gödel numbers of sentences in $T$ is not recursively decidable.
  \qed
\end{proof}

\begin{lemma}{}{qhat}
  Let $\hat{Q}$ be the conjunction of the seven axioms of Q.
  The set of $\L_A$-sentences of the form $\hat{Q} \to A$ is recursively decidable.
\end{lemma}
\begin{proof}
  \emph{Proof.}
  Let $P$ be the property of coding sentences of the form $\hat{Q} \to A$.
  $P(n)$ can be defined as $\exists y \leq n\, (\mathrm{Sent}(y) \land (n = \gn{\hat{Q} \to} * y))$.
  So $P(n)$ is (primitive) recursive.
  \qed
\end{proof}
...
\begin{proof}
  ...
  Q is recursively undecidable.
  As in the previous proof,
  let $\hat{Q}$ be the conjunction of Q's axioms.
  If the set of valid first-order sentences were recursively decidable,
  the set of valid $\L_A$-sentences of the form $\hat{Q} \to A$ would also be recursively decidable,
  by Lemma~\ref{lem:qhat} and the fact that the intersection of two recursively decidable sets is recursively decidable.
  By the soundness and completeness of first-order logic,
  $\hat{Q} \to A$ is valid iff $\proves_Q A$.
  So Q would be recursively decidable,
  contradicting what we just established.
  \qed
\end{proof}
...
\begin{exercise}
  Explain why there can be no recursive bound on the length of
  a proof for a sentence in the first-order calculus:
  for every recursive function $f$,
  there is a sentence with length $n$ that is provable,
  but whose proof requires more than $f(n)$ lines.
\end{exercise}

Now for the strengthened version of the syntactic Incompleteness Theorem.
In Section~\ref{sec:decidable} (Proposition~\ref{prop:complete-decidable}),
I showed that
every axiomatizable and complete first-order theory is decidable.
Together with Lemma~\ref{lem:undecidable},
this immediately gives us the what we seek:
every axiomatizable and consistent theory in which all recursive functions are representable is incomplete.

However,
Proposition~\ref{prop:complete-decidable} used the informal concept of computable axiomatizability;
so this argument relies on the Church-Turing Thesis.
Let's prove a parallel result for recursive axiomatizability.
...
\begin{proof}
  \emph{Proof.}
  Let $T$ be a recursively axiomatizable and complete first-order theory.
  As in the previous section,
  let $\mathrm{Prf}_T$ be the relation that holds between numbers $n$ and $m$
  iff $n$ codes a deduction of $m$ from some recursively decidable set of axioms for $T$.
  We know that $\mathrm{Prf}_T$ is recursive.
...
\begin{theorem}{Also Gödel's First Incompleteness Theorem}{goedel1c}
  Every consistent and recursively axiomatizable theory in which
  all recursive functions are representable is incomplete.
\end{theorem}
\begin{proof}
  \emph{Proof.}
  Let $T$ be a consistent and recursively axiomatizable theory in which
  all recursive functions are representable.
...
Since all recursive functions are representable in every extension of Q
(Theorem~\ref{thm:Q-rep}),
Theorem~\ref{thm:goedel1c} is often stated as saying that
every consistent and recursively axiomatizable extension of Q is incomplete.
We can prove an even stronger result
by strengthening Lemma~\ref{lem:undecidable}.

\begin{lemma}{}{undecidable2}
  Every $\L_A$-theory consistent with Q is recursively undecidable.
\end{lemma}
\begin{proof}
  \emph{Proof.}
  Let $T$ be an $\L_A$-theory consistent with Q,
  and suppose for reductio that $T$ is recursively decidable:
  the set $W$ of Gödel numbers of sentences in $T$ is recursive.
  Since $\mathrm{diag}$ is recursive,
  so is the property $P$ that holds of a number $x$ iff
  $\mathrm{diag}(x)$ is not in $W$.
  By exercise~\ref{ex:weak-repr}.(c),
  all recursive relations are weakly representable in any $\L_A$-theory
  consistent with Q.
...
\begin{theorem}{}{goedel1d}
  Every recursively axiomatizable $\L_A$-theory that is consistent with Q is incomplete.
\end{theorem}
\begin{proof}
  \emph{Proof.}
  Let $T$ be a recursively axiomatizable $\L_A$-theory that is consistent with Q.
  By Lemma~\ref{lem:undecidable2},
  $T$ is recursively undecidable.
  By Lemma~\ref{lem:complete-re},
  it follows that $T$ is incomplete.
  \qed
\end{proof}
...
Let's take stock.
Since all recursive functions are representable in PA,
...
we needed multiplication to define the recursive functions and relations.
Without multiplication, 
$\mathrm{Prf}_T$ and $*$ are no longer definable.
As a consequence,
...
the computational properties of the relations defined by the relevant formulas.
...
In fact, every recursive relation is definable by a $\Sigma_{1}$-formula.
But not every relation defined by a $\Sigma_{1}$-formula is recursive.

Some are just recursively enumerable.
A relation $R$ is \emph{recursively enumerable}
if there is a recursive relation $S$
such that $R(x_1,\ldots,x_n)$ holds iff $\exists y \, S(x_1,\ldots,x_n,y)$.
By the Church-Turing Thesis,
the recursively enumerable relations are precisely the computably enumerable relations.
(See Propositions~\ref{prop:re-existential} and \ref{prop:decidable-from-re} in Section~\ref{sec:decidable}.)

\begin{theorem}{}{rec-sigma1}
  A relation is recursively enumerable iff is definable in $\L_A$ by a $\Sigma_{1}$-formula.
\end{theorem}
\begin{proof}
  \emph{Proof sketch.}
  ...
  From right to left,
  assume that $R$ is defined by a $\Sigma_{1}$-formula $\exists y \, A(x,y)$,
  where $A$ is $\Delta_{0}$.
  ...
  we need show that every recursive relation is defined by a $\Sigma_{1}$-formula.
  Since prefixing existential quantifiers to a $\Sigma_{1}$-formula yields another $\Sigma_{1}$-formula,
  the result extends to every recursively enumerable relation.

  The proof that every recursive function is defined by a $\Sigma_{1}$-formula
  proceeds by induction on the construction of recursive functions.
  In chapter~\ref{ch:representability},
  I showed that the base functions (zero, successor, projection) are definable in $\L_A$,
  and that definability-in-$\L_A$ is closed under composition, primitive recursion, and minimization.
  By going through each part of this proof,
  we can check that
  the defining formulas are all $\Sigma_{1}$.
  This is obvious for the base functions,
  which I explicitly defined using $\Delta_{0}$-formulas.
  (For example, I showed that zero is defined by $x = 0$.)
  Closure under composition is also straightforward.
  I showed that
  $\mathrm{Cn}[f,g_1]$ is defined
  by $\exists v_1(F(y,v_1) \land G_1(v_1,x_1,\ldots,x_n))$.
  Since any initial existential quantifiers in $F$ and $G_1$
  can simply be pulled to the front,
  so the whole formula is $\Sigma_{1}$ if $F$ and $G_1$ are.

  Regular minimization requires a more work.
  I showed that $\mathrm{Mn}[f]$ is defined by
  ...
  one can show that it is definable by a $\Delta_{0}$-formula.

  Finally, for primitive recursion,
  I showed that $\mathrm{Pr}[f,g_1]$ is defined by
  $\exists c(\smallcaps{Seq}(c,x,k) \land \smallcaps{beta}(c,s(k),y))$,
  where $\smallcaps{Seq}(c,x,k)$ is defined in terms of $F$ and $G_1$ and $\smallcaps{beta}$.
  I've already mentioned that $\smallcaps{beta}$ is definable by a $\Delta_{0}$-formula.
  Using the beta function trick that we've just used for minimization,
  one can show that $\smallcaps{Seq}(c,x,k)$ is definable by a $\Sigma_{1}$-formula,
...
Since $\mathrm{Prov}_{\mathrm{PA}}$ is defined by existential quantification from the recursive relation $\mathrm{Prf}_{\mathrm{PA}}$,
it is recursively enumerable.
By Theorem~\ref{thm:rec-sigma1},
it is definable by a $\Sigma_{1}$-formula.
...
\begin{theorem}{The MRDP Theorem}{mrdp}
  A relation is recursively enumerable iff it is definable in $\L_A$ by a formula of the form $\exists x_1\ldots \exists x_n \; t_1 = t_2$.
\end{theorem}
...
the MRDP Theorem shows that every recursively enumerable relation is expressed by a formula
...
every sound and recursively axiomatizable $\L_A$-theory is incomplete.
It is easy to see that a theory is recursively axiomatizable
iff the set of Gödel numbers of its members is recursively enumerable.
Theorem~\ref{thm:goedel1} therefore applies to all theories
whose members are defined by a $\Sigma_{1}$-formula.
We can extend the result to non-axiomatizable theories that
are only definable by $\Pi_{2}$-formulas or $\Sigma_{12}$-formulas.

Let's say that a theory $T$ is \emph{definable in $\L_A$} if
there is an $\L_A$-formula $W(x)$ such that
for all sentences $B$, ${\Mod{N} \satisfies W(\gn{B})}$ iff $B \in T$.

\begin{theorem}{}{goedel1e}
  Every sound $\L_A$-theory that is definable in $\L_A$ is incomplete.
\end{theorem}
\begin{proof}
  \emph{Proof.}
  If $T$ is sound and complete then $T = \mathrm{Th}(\Mod{N})$.
  By Theorem~\ref{thm:tarski2},
  $\mathrm{Th}(\Mod{N})$ is not definable in $\L_A$.
  So if $T$ is sound and definable in $\L_A$
  then it is incomplete.
  \qed
\end{proof} 

\begin{exercise}
  Explain why a theory is recursively axiomatizable iff
  the set of Gödel numbers of its members is recursively enumerable.
  (Hint: if $T$ is recursively axiomatizable then $\mathrm{Prf}_T$ is recursive.)
\end{exercise}
