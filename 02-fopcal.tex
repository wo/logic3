...
A variable is \emph{free in} a formula if it has at least one free occurrence in it.
A formula in which some variable is free is \emph{open}.
A formula that isn't open is \emph{closed}.
A \emph{sentence} is a closed formula.

\begin{exercise}
  Give an example of a formula in which the variable $x$ is free even though
  not all occurrences of $x$ in the formula are free.
\end{exercise}

\begin{exercise}
  Assume that `$F$' is a 1-ary (= \emph{monadic}) predicate symbol, `$a$' is a constant, and `$x$' a variable.
...
`$A(c)$' is then shorthand for `$A(x/c)$'.
...
\begin{flalign*}
  ...
  \quad 4.\quad & \forall x (A \to B), A \vdash A && \text{(Id, Mon)} & \\
...
individual constants pick out objects in the intended domain of discourse;
...
\begin{definition}{}{model1}
  A \emph{model} $\Mod{M}$ for a first-order language $\L_{1}$ consists of
  \begin{cenumerate}
    ...
    \item[(ii)] an \emph{interpretation function} $I$ that assigns
      ...
      to each $n$-ary predicate symbol of $\L_{1}$ a set of $n$-tuples from $D$.
  \end{cenumerate}
\end{definition}
...
and so on.
The set assigned to a predicate symbol is called the \emph{extension} of the predicate symbol.
...
$c$ is an individual constant and $P$ a predicate symbol,
...
that is exactly like $\Mod{M}$ in every respect other than the interpretation of $c$,
...
we also say that $\Mod{M}$ is \emph{a model of} $A$,
or that $\Mod{M}$ \emph{satisfies} $A$.
...
let's introduce a predicate symbol for identity.
We'll use `='.
...
prevented us from having a predicate symbol `='.
...
I want to introduce the second addition to our definition of first-order languages.
It allows forming complex terms like `$1+2$'.
...
takes a number $x$ as input and returns
...
\begin{exercise}
  Give an example of a function with three arguments.
  (That is, the function takes three objects as input.)
\end{exercise}
...
\begin{definition}{}{terms}
  ...
  \begin{cenumerate}
    \item[(i)] Every variable and every individual constant is a singular term.
    \item[(ii)] If $f$ is an $n$-ary function symbol and $t_1,\ldots,t_n$ are singular terms then $f(t_1,\ldots,t_n)$ is a singular term.
  \end{cenumerate}
\end{definition}
%
...
But we have two new axiom schemas for identity:
...
\begin{definition}{}{model=}
  ...
  \begin{cenumerate}
    ...
    \begin{itemize*}
      ...
      \item to each non-logical $n$-ary predicate symbol of $\L_{1}^=$ a set of $n$-tuples from $D$.
    \end{itemize*}
  \end{cenumerate}
\end{definition}
...
\begin{exercise}\label{ex:domain-size}
  Construct three sentences $A_{1}, A_{2}, A_{3}$ with `=' as the only predicate symbol
  such that (a) $A_1$ is true in a model $\Mod{M}$ iff the domain of $\Mod{M}$ has at least two members, (b) $A_2$ is true in $\Mod{M}$ iff the domain of $\Mod{M}$ has at most two members, (c) $A_3$ is true in $\Mod{M}$ iff the domain of $\Mod{M}$ has exactly two members.
\end{exercise}
...
If $t$ is empty,
should we say that $t=t$ is true?
...
We dodge the questions by banning empty terms.
...
it plays no role in clause (v) of definition~\ref{def:satisfaction=}.
A similar point applies to the other reason why we need an unending supply of constants.
In our first-order calculus,
deriving a universal statement $\forall x B(x)$ from another universal statement $\forall x A(x)$
often requires instantiating $\forall x A(x)$ to $A(c)$,
deriving $B(c)$,
...
to play certain internal roles in the handling of quantifiers in our proof system and semantics.
Conceptually,
the constants we need resemble variables.
...
\begin{definition}{Isomorphic Structures}{isomorphic}
  Structures $\Mod{N}$ and $\Mod{B}$ are \emph{isomorphic} if there is a bijection $f$ from the domain of $\Mod{N}$ to the domain of $\Mod{B}$ such that xxx
\end{definition}
...
\begin{proof}
  ...
  $\Mod{M}^{ed}$ and $\Mod{M}^d$ agree on all symbols in $B(x/t_2)(y/d)$.
  So by the coincidence lemma,
  \begin{equation}\tag{8}
    \Mod{M}^d \satisfies B(x/t_2)(y/d) \text{ iff } \Mod{M}^{ed} \satisfies B(x/t_2)(y/d).
  \end{equation}
  By induction hypothesis,
  \begin{equation}\tag{9}
...
\begin{exercise}
  ...
  it is impossible to prove $\bot$ in the first-order calculus.
  Recall that a calculus is Post-complete if
  any addition of a new axiom schema
  (that is not already provable in the calculus)
  makes the calculus inconsistent.
  Can you outline a proof showing that the first-order calculus is not Post-complete?
\end{exercise}
