...
We'll introduce a genuine rule -- called /(Universal) Generalization/ -- for the
converse inference, from $A(x/c)$ to $\forall x A$.

\begin{principles}
    \pri{Gen}{\text{From }A(x/c)\text{ one may infer }\forall x A.}
\end{principles}
...
predicate calculus. That is: $A$ can be derived from some instances of A1-A5 by
MP and Gen.
...
We'll add some new axiom schemas for identity: for any terms $t_{1}$ and $t_{2}$ and formulas $A$ in which only $x$ is free,

\begin{principles}
\pri{SI}{t_1=t_2 \text{for any closed terms $t_1$ and $t_2$}}\\
\pri{LL}{t_1 = t_2 \to (A(x/t_1 )\to A(x/t_2))}
\end{principles}
