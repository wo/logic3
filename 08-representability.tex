...
\end{definition}
\noindent%
Definition~\ref{def:expresses-function} could be extended to partial functions,
...
$\t{\mathrm{fac}(0), \mathrm{fac}(1), \mathrm{fac}(2), \ldots}$.
Our formula $F(x,y)$ will say ``$y$ is the last element of the sequence $\t{\mathrm{fac}(0), \ldots, \mathrm{fac}(x)}$''.
...
$z$ codes the sequence $\t{\mathrm{fac}(0), \mathrm{fac}(1), \ldots, \mathrm{fac}(x)}$.
From this,
we can express $F(x,y)$ as $\exists z(\smallcaps{Seq}(z,x) \land \smallcaps{Entry}(z,s(x),y))$.
This says that
there is a number $z$ that codes $\t{\mathrm{fac}(0), \ldots, \mathrm{fac}(x)}$
...
and so $\mathrm{fac}(x)$ is the $s(x)$-th entry.)
