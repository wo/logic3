...
need to call identity projection.

need to define: regular minimization

need: a relation is recursive iff its characteristic function is recursive
...
If f and g is computable, then h is also computable:
to compute $h(x)$,
we only need to compute $g(x)$ and feed that value into $f$.
...
All p.r. functions are total.
We can see this by induction on their structural definition.
The base functions are total.
The composition of total functions is total.
The primitive recursion of total functions is total.
...
expression
$(μk \leq p)R(k)$
denotes the least number $k \leq p$ such that R holds of k, if there is such a number, and denotes p otherwise.

This can also be thought of as a bounded search operation.
The computation basically goes through all $k$ from 0 to $p$
until it finds one for which $R(k)$ holds;
it than returns that value of $k$.

$\mu$ successively tests 0,1,2,\ldots until the test condition is satisfied.
In programming languages,
this corresponds to a `for' loop that
iterates through a fixed list of numbers.

This can be defined by recursion like this:
Let $f(x,p)$ be the target function $\mu k \leq p [r(x,k)=0]$.
We define $f(x,0) = 0$ and
$f(x,n+1) = f(x,n)$ if this is leq $n$,
otherwise $f(x,n+1) = n$ if $f(x,n)=0$,
otherwise $f(x,n+1) = n+1$. [??]
...
But Gödel realized that
these can't be all the computable functions. We can diagonalize out.

The antidiagonal is computable, but not primitive recursive.

Why not?
Think of how we'd compute its value for input n.
We enumerate the pr functions up to n.
Now we have a description of an algorithm,
which we need to run until it halts.
We can't tell how long this will take.
So we need an open-ended loop.
...
The proof of Goodstein's Theorem assigns to each item in the sequence a certain transfinite ordinal, in a rather direct way,
and shows that successive ordinals always decrease.
Since there is no infinite descending sequence,
it follows that the sequence must reach 0 after finitely many steps.
...
What we need is unbounded search.
We have an operation that searches
through all numbers up to a given bound.
We'll now introduce an operation that searches
through all numbers until it finds a number $x$ for which a given condition $C(x)$ is satisfied.
If there is no such number, the operation returns nothing.
(It ``runs forever''.)
Since we're defining operations on functions,
the relevant condition $C(x)$ will be that some other function $f$ returns
some desired value $k$ when given $x$ as input,
possibly in addition to other inputs.
We can always reduce the case of $k$ to the case of 0.
...
Example (Enderton): the one-place function that is defined for no input is recursive.
How is it defined? E.g. by minimization from g(x,y) = 1.
...
What if we diagonalize out of the general recursive functions?
We can define $d(x)$ to output the $x$'th function applied to $x$, plus 1.
If $d$ is the $x$'th function and outputs a number,
we get a contradiction.
But no contradiction arises if $d$ is the $x$'the function and outputs nothing.
...
The theory of computable functions I've described
doesn't just sort the functions into computable and uncomputable.
It also assigns a ``number of steps'' to each computable function,
depending on the input.
Or rather,
it assigns such a number to each way of defining a computable function
in terms of the base functions and operations.
Any such definition in effect describes an algorithm for computing the function.
The (input-relative) step count for an algorithm is called the algorithm's \emph{computational complexity}.

We can also have define computational complexity in terms of Turing machines.
(This is, in fact, the more common definition.)
The computational complexity of a Turing machine program is
given by the number of steps it takes the machine to return an output,
which generally depends on the input.
Technically,
then,
computational complexity is a function from an input to a number (a step count).

In the field of complexity theory,
people study the general form of this function.
For example,
...
the number of steps bound by a polynomial function of the input size.
For example,
if the input has size $n$,
the number of steps might be bound by $n^{2}$,
or by $10 n^{10} + 3 n^{7}$.

The class of polynomial-time algorithms is very natural class.
It doesn't depend on details of how we count steps or the length of an input,
and it is closed under composition and ``subroutine insertion'',
wherein an arbitrary part of an algorithm is replaced by another algorithm.
In analogy to the Church-Turing theses,
the polynomial-time algorithms are often identified with the \emph{feasible} algorithms,
where `feasible' is an informal, pre-theoretic concept.

Consider, for example, the task of checking
a given sentence of propositional logic is true relative to
a given assignment of truth-values to sentence letters.
This can be done in polynomial time.
By contrast,
consider the task of checking whether a sentence of $\L_{0}$
is true relative to \emph{any} assignment of truth-values
(i.e., whether it is satisfiable).
The obvious algorithm of trying out all possible assignments
is exponential in the number of sentence letters.
For 100 sentence letters,
it requires over $10^{100}$ steps,
more than the number of atoms in the observable universe.
This algorithm is not feasible.

Is there a feasible, polynomial-time algorithm for
deciding whether a sentence of $\L_{0}$ is satisfiable.
Nobody has yet found such an algorithm,
and it is generally believed that none exists.
But oddly,
we don't know for sure.
This is an instance of the notorious \emph{P vs NP problem},
widely regarded as the most important open problem in computer science.



\section{The MRDP theorem}
