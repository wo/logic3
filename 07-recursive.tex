...
Let's concentrate on functions on natural numbers.
...
\begin{cenumerate}
  ...
  \item The \emph{zero function} $z$ returns 0 for any input number.
...
\begin{exercise}
  Define the 1-place function that always returns 4, using composition and the base functions.
\end{exercise}
...
could be stated more explicitly as follows:
...
two other functions, $f$ and $g$:
$f$ specifies the starting point, $h(x_1,\ldots,x_n,0)$;
$g$ specifies $h(x_1,\ldots,x_n,s(y))$ in terms of $h(x_1,\ldots,x_n,y)$.
We allow $g$ to also depend on $x_1, \ldots, x_n$, and $y$.
So the general format for primitive recursion looks like this:
\begin{align*}
...
For another example,
consider the \emph{truncated predecessor} function $\mathrm{pred}$ that
returns the predecessor of a number if that number is positive,
and otherwise 0.
This can be defined by primitive recursion as follows:
\begin{align*}
  \mathrm{pred}(0) &= 0\\
  \mathrm{pred}(s(y)) &= y
\end{align*}
...
Using $\mathrm{pred}$,
we can define a \emph{truncated difference} function:
\begin{align*}
  x \dotminus 0 &= x\\
...
Another useful function is \textit{switcheroo} function $\delta$ (``delta'') that
...
%
\begin{exercise}
  Write the definitions of $\mathrm{pred}$, $\dotminus$, and $\delta$ using the $\mathrm{Pr}$ notation.
\end{exercise}

\begin{exercise}
  Define the multiplication function using primitive recursion.
  Can you express your definition using the $\mathrm{Pr}$ notation?
...
that maps any member of the set to 1 and any non-member to 0.
At the moment,
we are interested in sets whose members are numbers or tuples of numbers.
For example,
the set of odd numbers is primitive recursive,
as its characteristic function $\chi$ can be defined by primitive recursion, as follows:
\begin{align*}
  \chi(0) &= 0\\
  \chi(s(y)) &= \delta(\chi(y))
\end{align*}
The property of being odd is also primitive recursive.
A property is a 1-place relation.
So a property of numbers is primitive recursive iff
there is a primitive recursive function that maps every number that has the property to 1 and every other number to 0.

An example of a 2-place primitive recursive relation is the less-than-or-equal relation on $\mathbb{N}$.
...
\begin{proof}
  \emph{Proof}.
  Immediately from exercises~\ref{ex:pr-negation} and~\ref{ex:pr-conjunction}. \qed
...
\begin{proof}
  ...
  the generalization to more arguments is straightforward.
...
\begin{align*}
  \mathrm{max}(x,y) &= x \cdot \mathrm{Leq}(y,x) + y \cdot \delta(\mathrm{Leq}(y,x))\\
                    &= x \cdot \delta(x\dotminus y) + y \cdot \delta(\delta(x\dotminus y)).
\end{align*}
...
\begin{proof}
  ...
  \[
    \mu y \leq x \,R(x,y) = \begin{cases}
      ...
      \,0 & \text{otherwise.} \hspace{17em} \qed
    \end{cases}
  \]
\end{proof}

These operations give us a powerful toolkit for defining primitive recursive functions and relations.
...
\begin{align*}
  \mathrm{pri}(0) &= 2,\\
  \mathrm{pri}(s(y)) &= \mu x \leq \mathrm{pri}(y)! + 1 \,(\mathrm{Prime}(x) \land x > \mathrm{pri}(y)).
\end{align*}
Here I use Euclid's observation that if $p$ is prime
then the next prime is no greater than $p! + 1$.
...
For example,
$\mathrm{entry}(2^3 \cdot 3^2 \cdot 5^1 \cdot 7^4, 1) = 2$.
...
\]
Thus $\mathrm{len}(2^3 \cdot 3^2 \cdot 5^1 \cdot 7^4) = 4$.

\begin{exercise}
  What are $\mathrm{entry}(1, 0)$ and $\mathrm{len}(1)$?
\end{exercise}
...
(with, say, a single argument):
we start with the base functions,
then we list all (one-place) functions that can be obtained from these by one application of composition or primitive recursion,
...
We would first identify the $n$-th primitive recursive function $f_n$.
...
We've briefly met such an operation above,
in the form of the unbounded minimization operator `$\mu$':
$\mu x \,P(x)$ is the least number $x$ for which $P(x)$ holds.
We'll introduce a version of this operation that applies to functions rather than relations.

Given a two-place function $f(x,y)$,
we can define a 1-place function $h$
...
We can also apply minimization to partial functions,
...
if the computation of $f(x,z)$ never halts,
the search never proceeds beyond $z$.
We therefore stipulate that
if $f$ is an arbitrary $n+1$-place function, then
...
\begin{proof}
  ...
  I write $\bar x$ for $x_1, \ldots, x_n$.
  ...
  \emph{Composition.}
  Suppose we have Turing machines for computing $g_1,\dots,g_m$ and $f$.
  We can design a machine for computing
  $h = \mathrm{Cn}[f,g_1,\dots,g_m]$ on any input $\bar x$ as follows.
  The machine first calls each $g_i$ machine (as a subroutine) on input $\bar x$,
  and stores the results next to each other, separated by blanks.
  It then calls the $f$ machine on this pattern of strokes and blanks.
  The output is $f(g_1(\bar x), \ldots, g_m(\bar x))$.

  \emph{Primitive recursion.}
  Suppose we have Turing machines for computing $f$ and $g$.
  Let $h = \mathrm{Pr}[f,g]$.
  That is, $h(\bar x,0)=f(\bar x)$ and $h(\bar x,y{+}1)=g\big(\bar x,y,h(\bar x,y)\big)$.
  The machine for computing $h$ works as follows.
  Given input $\bar x,y$,
  it first calls the $f$-machine on $\bar x$ and stores the result in a block that will eventually hold $h(\bar x,y)$;
  ...
  Otherwise it calls the $g$ machine on $\bar x$, the current counter value, and the current result block,
  ...
  At that point,
  the result block will contain $h(\bar x,y)$.

  \emph{Minimization.}
  Suppose we have a Turing machine for computing $f$.
  Let $h = \mathrm{Mn}[f]$.
  that is, $h(\bar x)=\mu y\,[\,f(\bar x,y)=0\,]$.
  ...
  In each iteration,
  it runs the machine for $f$ on $\bar x$ and the current $y$ block.
  If the output is $0$,
  the machine halts and erases everything except the $y$ block.
  Otherwise,
  the machine adds a stroke to the $y$ block
  and enters the next iteration of the loop.
  If there is some $y$ such that $f(\bar x,y)=0$
  and $f(\bar x,z)$ is defined for all $z<y$,
...
\begin{theorem}{(Kleene's Normal Form Theorem)}{kleene-normal}
...
Since the Turing-computable functions and the recursive functions coincide,
...
or as the claim that
every computable function is Turing-computable.
The two claims are equivalent.

\begin{exercise}
  Is the Busy Beaver function (section~\ref{sec:tm-uncomputability}) recursive?
\end{exercise}

\begin{exercise}
  Using the Church-Turing thesis,
  explain why the set of regular recursive functions is not decidable.
  Is it computably enumerable?
\end{exercise}
