...
\begin{cenumerate}
  ...
  \item For each $n$ and $i$, the \emph{projection function} $\pi^{n}_{i}$ takes $n$ numbers as input and return the $i$-th of them. (For example $\pi^{3}_{2}(5,9,2) = 9$.)
\end{cenumerate}
\noindent%
...
\begin{align*}
  \mathrm{fac}(0) &= 1\\
  \mathrm{fac}(s(y)) &= s(y) \cdot \mathrm{fac}(y)
\end{align*}
...
it doesn't directly account for our definitions of $\mathrm{pred}$, $\mathrm{fac}$, and $\delta$,
...
\]
This trick also works for $\mathrm{fac}$ and $\delta$,
...
\begin{proof}
  ...
  \]
  is primitive recursive and defines $h$. \qed
\end{proof}
