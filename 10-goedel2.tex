...
A \emph{(recursively) axiomatizable} theory is one for which
...
\noindent%
If $T$ is sound,
the formula $\smallcaps{Prf}_T(x,y)$ also defines $\mathrm{Prf}_T$ in $\L_A$,
so that
\begin{equation*}
...
Now let $T$ be some recursively axiomatizable extension of Q.
...
he claimed that
no sufficiently strong, recursively axiomatizable, and consistent theory
...
By (1),
it can then infer $G$.
But we've assumed that $T$ is a recursively axiomatizable extension of Q.
And we know from Gödel's First Incompleteness Theorem that
no recursively axiomatizable and \emph{consistent} extension of Q can prove $G$.
So $T$ must be inconsistent.

In sum,
if $T$ is a consistent and recursively axiomatizable extension of Q,
...
\begin{exercise}\label{ex:ref}
  Explain why it doesn't follow from the previous exercise that $\smallcaps{Prov}_{\mathrm{PA}}(x)$ satisfies this condition:
  \vspace{-0.2em}
  \begin{axioms}
...
\begin{theorem}{Gödel's Second Incompleteness Theorem}{goedel2}
  If $T$ is a consistent and recursively axiomatizable theory in which $\mathrm{diag}$ is representable,
...
This theory is still recursively axiomatizable,
...
\end{equation}
This is called the \emph{Henkin sentence} for $T$.
...
Löb showed that if $T$ is consistent, recursively axiomatizable, and sufficiently strong,
...
\begin{theorem}{(Löb's Theorem)}{Löb's Theorem}
  If $T$ is a consistent, recursively axiomatizable theory in which $\mathrm{diag}$ is representable,
...
Assume that $T$ is a recursively axiomatizable theory in which $\mathrm{diag}$ is representable.
...
\begin{exercise}
  ...
  (a) Explain why $\proves_{\mathrm{PA}^+} \smallcaps{Prov}_{\mathrm{PA}^+}(\gn{\bot})$.
  (b) Show that there is a sentence $A$ for which $\proves_{\mathrm{PA}^+} \neg(\smallcaps{Prov}_{\mathrm{PA}^+}(\gn{A}) \to A)$.
\end{exercise}
...
let me quickly introduce some background.

Propositional modal logic
is an extension of classical propositional logic.
The language $\L_M$ of propositional modal logic
is obtained from $\L_0$ by adding the unary sentence operator `$\Box$'.
Thus
...
however.
It has another axiom schema, known as GL (for Gödel-Löb):
...
\begin{exercise}
  ...
  Show that $\Box \bot$ is provable in GL+Ref.
  Is GL+Ref consistent?
\end{exercise}
...
\begin{exercise}
  % The proof of Theorem~\ref{th:goedel2} shows that
  % $\proves_{GL} (G \leftrightarrow \neg \Box G) \to (\neg \Box \bot \to G)$.
  Show that
  $\proves_{GL} (G \leftrightarrow \neg \Box G) \to (G \leftrightarrow \neg\Box \bot)$.
  (What does this tell us about the Gödel sentence $G$?)
  % Proof idea:
  % If G then
  %    ¬□G (by D)
  %    □⊥ → □G (from ⊢ ⊥ → G)
...
In essence,
it says that
for any recursively axiomatized and consistent theory
...
(With a concrete coding of programs and patterns,
it can easily be shown to be recursive.)
...
\begin{theorem}{Chaitin's Incompleteness Theorem}{chaitin}
  If $T$ is a consistent, recursively axiomatizable extension of Q,
