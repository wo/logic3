...
No.
Consider the theory $\mathrm{Th}(\Mod{N})$
that contains all true sentences of $\L_{A}$
-- where by `true' I mean true in the standard model of arithmetic $\Mod{N}$
...
The proof of Theorem \ref{thm:nonstandardPA} works just as well for $\mathrm{Th}(\Mod{N})$.
There is no way to pin down the intended model of arithmetic
in a first-order language.

\begin{exercise}
  The upward Löwenheim-Skolem theorem also implies that there are nonstandard models of $\mathrm{Th}(\Mod{N})$.
  Explain.
\end{exercise}
...
define a model of arithmetic (of $\mathrm{Th}(\Mod{N})$ and PA and Q)
