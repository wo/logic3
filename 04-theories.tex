...
\chapter{Theories}\label{ch:theories}

In this chapter,
we take a look at first-order theories:
sets of sentences in a formal, first-order language that are
assumed to describe a particular domain of objects.
A theory might describe the behaviour of physical systems
or moral norms,
...
They also formalized the exact assumptions that
were needed to derive well-known results.
These assumptions were collected into \emph{axioms} for the relevant area of maths.
...
(The variable $x$ must not occurr in $t_1$ or $t_2$.)
%
% \begin{axioms}
...
$a, s(a), s(s(a)), \ldots$.
The following axiom rules out such additional chains,
...
however.
We'd like to have an axiom saying that
...
and then define its value for any successor number
in terms of its value for the previous number.
For example,
...
Are all truths in the language of arithmetic entailed by the axioms of PA?
For a while,
...
\begin{exercise}
  ...
  (c) $\forall x \forall y(x + y = y + x)$.
    % By induction on y. Base: x + 0 = x = 0 + x.
    % Step: x + s(y) = s(x + y) = s(y + x) = s(y) + x.
\end{exercise}
...
\begin{exercise}
  ...
  \begin{cenumerate}
    ...
    define a model $\Mod{M}$ of Q1--Q7 with two objects $a$ and $b$
    ...
    \item[(c)] Is $\forall x \forall y (x+y = y+x)$ true in your model $\Mod{M}$?
    If yes, change the interpretation of $+$ to make it false while keeping Q1--Q7 true.
  \end{cenumerate}
\end{exercise}


\section{Set theory}\label{sec:zfc}
...
But not otherwise:
a set can only appear at a stage after all its elements have appeared.
...
and so on.
This yields an infinite sequence $V_0, V_1, V_2, \dots$ of ever-larger sets,
...
we construct $V_{\omega \cdot 2 + 1}$, $V_{\omega \cdot 2 + 2}$, etc.\ by taking power sets.
...
To define the union operation $\cup$,
for example,
...
Replacement is needed to ensure that
the cumulative hierarchy extends beyond the finite stages $V_0, V_1, V_2, \ldots$.
From Z1--Z8 (sometimes called \emph{Zermelo set theory} or Z),
we get the finite stages,
but we can't prove the existence of $V_{\omega}$.
With Replacement,
we can show that there is a set $\{ V_{n} : n \in \mathbb{N} \}$ of all finite stages:
the formula $A(x,y)$ in Z9 says that
$y$ is obtained from $\emptyset$ by $x$ applications of the power set operation.
$V_{\omega}$ is the union of $\{ V_{n} : n \in \mathbb{N} \}$.
...
\begin{exercise}
  The axioms of ZFC guarantee that for any three things a,b,c, there is a set \{a,b,c\}.
  Explain how.
\end{exercise}
...
ZFC proves that $\mathcal{P}(\omega)$ is uncountable by proving that
